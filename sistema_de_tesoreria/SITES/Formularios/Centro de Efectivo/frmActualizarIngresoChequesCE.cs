...
{
    public partial class frmActualizarIngresoChequesCE : Form
    {
        ...
        public frmActualizarIngresoChequesCE(ref BindingList<ChequeDeposito> listachequedeposito, ref Colaborador colaborador, ref Monedas monedadeposito, Boolean readOnly)
        {
            InitializeComponent();
        ...
        {
            gbregistrocheque.Enabled = readOnly;
            btnQuitarCheque.Enabled = readOnly;
            btnIncluir.Enabled = readOnly;
            ...
            try
            {
                if (validarcamposagregar())
                {
                    ...
                    if (dgvcheques.SelectedRows.Count != 0)
                    {
                        ...
                        cheque.ID = Int32.Parse(dgvcheques.SelectedRows[0].Cells["ID"].Value.ToString());
        ...
        private void dgvcheques_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvcheques.SelectedRows.Count > 0)
            {
                ...
                nudMonto.Value = (decimal)(dgvcheques.SelectedRows[0].Cells["Monto"].Value);
            }
            else
            {
                limpiarcampos();
